...
if __name__ == '__main__':
    problem = TestProblem(noise_std = 0.001, negate=False).to(**tkwargs)
    # problem = TestProblem_Branin(noise_std = 0.001, negate=True).to(**tkwargs)
    weights = torch.tensor([0.5, 0.5]).to(**tkwargs)
    ...
    # mtgp_results = torch.log(-torch.stack(mtgp_trial_objectives)[:, n_init:]).cummin(1).values
    # batch_results = torch.log(-torch.stack(batch_trial_objectives)[:, n_init:]).cummin(1).values
    # random_results = torch.log(-torch.stack(rand_trial_objectives)[:, n_init:]).cummin(1).values

    mtgp_results = torch.log(torch.stack(mtgp_trial_objectives)[:, n_init:]).cummax(1).values
    batch_results = torch.log(torch.stack(batch_trial_objectives)[:, n_init:]).cummax(1).values
    random_results = torch.log(torch.stack(rand_trial_objectives)[:, n_init:]).cummax(1).values
    ...
    plt.title('Multi-task test problem 0.5*f(x,s) + 0.5*df/ds (x,s))')
    # plt.title('Multi-task test problem 1/2*(Branin(x) + Branin(x*1.1))')
    plt.savefig('Feb21_Synthetic.pdf')